...
var kartyaAdatok = [
    ...
    {id:4,value:4,sign:''},
    {id:5,value:3,sign:''},
    {id:6,value:0,sign:'varazslo'},
    {id:7,value:-6,sign:''},
    {id:8,value:6,sign:''},
    {id:9,value:0,sign:'aranybanya'},
    {id:10,value:2,sign:''},
...
function kartyahuzas(){
    if(vankartya == false && generaltkepek.length != 23){
        ...
        if(valaszto.length == 0){
            valaszto.push({
                src: random,
                kartyaAdatok: kartyaAdatok[random-1]
            })
        }else{
            valaszto[0] = ({
                src: random,
        ...
        //alert("Helyezd el a táblán a kihúzött kártyát elsőnek!");
    }else{
        document.getElementById("kartyabox").style.borderColor = "red";
        //alert("Nincs több kártya");
    }
    tkepborder()
}
var tpick;
var tornyok = [0,0,0,0]
var tornyok2 = [3,2,1,0]
function pick(div){
    tkepborder()
    var id = div.id;
    if(vankartya == true){
        //alert("Elsőnek rakd le a kihúzott kártyát!")
    }else{
        tpick = div.id; 
        ...
        if (id == "tkep4") {
            if(valaszto.length == 0){
                valaszto.push({
                    src: valaszto2[0].src,
                    kartyaAdatok: valaszto2[0].kartyaAdatok
                })
            }else{
                valaszto[0] = ({
                    src: valaszto2[0].src,
            ...
            if(valaszto.length == 0){
                valaszto.push({
                    src: 23+parseInt(id.slice(4))+1,
                    kartyaAdatok: kartyaAdatok[23+parseInt(id.slice(4))+1-1]
                })
            }else{
                valaszto[0] = ({
                    src: 23+parseInt(id.slice(4))+1,
    ...
    if(vankartya == false && tpick == null){
        ...
        if(foglaltcella.includes(div.id) == false){
            ...
            cellak.push({
                cellaid: div.id,
                src: valaszto[0].src,
                kartyaAdatok: valaszto[0].kartyaAdatok,
            })
...
function tablageneralas(){
    ...
    for (let index = 0; index < 5; index++) {
        ...
        for (let index2 = 0; index2 < 6; index2++) {
            ...
            oszlopDiv.setAttribute("onclick","cellavalasztas(this)");
            oszlopDiv.id = k    ;
    ...
    var oszlopDiv = document.createElement("div");
    oszlopDiv.className += " toronyvalaszto";
    oszlopDiv.id = "v2";
    document.getElementById("valaszto").appendChild(oszlopDiv);
    var oszlopDiv = document.createElement("div");
    oszlopDiv.className += " kartyavalaszto";
    oszlopDiv.id = "v1";
    document.getElementById("valaszto").appendChild(oszlopDiv);
    for (let index = 0; index < 4; index++) {
        var kep = document.createElement("img");
        kep.id = "tkep"+index;
        kep.src = "kartyak/"+(24+index)+".png";
...
function pontokcount(){
    ...
    for (let index = 0; index < 5; index++) {
        ...
        penz += (sor*torony);
        console.log(index,". sor:",sor*torony);
    }
    for (let index = 0; index < 6; index++) {
        ...
        penz += (oszlop*torony);
        console.log(index,". oszlop:",oszlop*torony);
    }
    coin(); 
}
var egyenleg = 50;
